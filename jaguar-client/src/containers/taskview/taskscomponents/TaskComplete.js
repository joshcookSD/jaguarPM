    ...
    render() {
        ...
        const _complete = async () => {
            ...
            await this.props.complete({
                variables: {_id: _id, iscompleted: !isComplete, completeddate: completeddate},
                refetchQueries: [{ query: updateQuery, variables: variables}]
            })
        };
        return (
            <List.Icon
                name={isComplete ? 'check circle' : pastDue ? 'alarm' : pastPlan ? 'warning sign' : 'square outline'}
                size='large'
                color= { isComplete? 'green' : pastDue ? 'red' : pastPlan ? 'yellow' : 'green'}
